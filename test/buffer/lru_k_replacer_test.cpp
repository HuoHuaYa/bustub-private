...
TEST(LRUKReplacerTest, DISABLED_SampleTest) {
  // Note that comparison with `std::nullopt` always results in `false`, and if
  // the optional type actually does contain a value, the comparison will
  // compare the inner value. See:
  ...
  // Add six frames to the replacer. We now have frames [1, 2, 3, 4, 5]. We set
  // frame 6 as non-evictable.
  ...
  // To break ties, we use LRU with respect to the oldest timestamp, or the
  ...
  // Insert new frames [3, 4], and update the access history for 5. Now, the
  // ordering is [3, 1, 5, 4].
  lru_replacer.RecordAccess(3);
  lru_replacer.RecordAccess(4);
  lru_replacer.RecordAccess(5);
  ...
  // Set 6 to be evictable. 6 should be evicted next since it has the maximum
  // backward k-distance.
  ...
  // Update the access history for frame 1 and make it evictable. Now we have
  // [4, 1].
  lru_replacer.RecordAccess(1);
  lru_replacer.RecordAccess(1);
  lru_replacer.SetEvictable(1, true);
  ...
  // There is nothing left in the replacer, so make sure this doesn't do
  // something strange.
  frame = lru_replacer.Evict();
  ASSERT_EQ(false, frame.has_value());
  ASSERT_EQ(0, lru_replacer.Size());

  // Make sure that setting a nonexistent frame as evictable or non-evictable
  // doesn't do something strange.
  lru_replacer.SetEvictable(6, false);
  lru_replacer.SetEvictable(6, true);
}
