...
TEST(TxnSerializableTest, SerializableTest) { // NOLINT
  fmt::println(stderr, "--- SerializableTest2: Serializable ---");
  {
    ...
    TxnMgrDbg("after two updates", bustub->txn_manager_.get(), table_info.get(),
...
TEST(TxnSerializableTest, ConcurrentSerializableTest) { // NOLINT
  fmt::println(stderr, "--- SerializableTest2: Concurrent Serializable ---");
  {
    for (int i = 0; i < 10; i++) {
      ...
      WithTxn(txn2, ExecuteTxn(*bustub, _var, _txn,
                               "UPDATE maintable SET a = 0 WHERE a = 1"));
      TxnMgrDbg("after two updates", bustub->txn_manager_.get(),
      ...
      commit_threads.emplace_back(
          [txn2, &bustub, &result_mutex, &success_cnt]() {
      ...
      commit_threads.emplace_back(
          [txn3, &bustub, &result_mutex, &success_cnt]() {
...
TEST(TxnAbortTest, SimpleAbortTest) { // NOLINT
  ...
  fmt::println(stderr, "A: INSERT then ABORT - ensure nothing persists");
  {
    ...
    WithTxn(txn1,
            ExecuteTxn(*bustub, _var, _txn,
                       "INSERT INTO maintable VALUES (1,10), (1,11), (2,20)"));
    WithTxn(txn1, QueryShowResult(*bustub, _var, _txn,
                                  "SELECT a, b FROM maintable ORDER BY a, b",
  ...
  }
  {
    ...
    WithTxn(txn_check,
            QueryShowResult(*bustub, _var, _txn,
                            "SELECT a, b FROM maintable ORDER BY a, b",
                            IntResult{}));
  ...
  fmt::println(stderr,
               "B: Prepare committed baseline for UPDATE/DELETE abort tests");
  {
    auto txn2 = BeginTxn(*bustub, "txn2");
    WithTxn(txn2, ExecuteTxn(
                      *bustub, _var, _txn,
                      "INSERT INTO maintable VALUES (1,10), (2,200), (3,300)"));
    WithTxn(txn2, CommitTxn(*bustub, _var, _txn));
  }
  {
  ...
  fmt::println(stderr, "C: UPDATE then ABORT - values should revert");
  {
    ...
    WithTxn(txn3, ExecuteTxn(*bustub, _var, _txn,
                             "UPDATE maintable SET b = 0 WHERE a >= 2"));
    WithTxn(txn3, QueryShowResult(*bustub, _var, _txn,
                                  "SELECT a, b FROM maintable ORDER BY a, b",
  ...
  fmt::println(stderr, "E: Commit with duplicates (no primary key) and verify");
  {
    ...
    WithTxn(txn5, ExecuteTxn(*bustub, _var, _txn,
                             "INSERT INTO maintable VALUES (1,999), (1,1000)"));
    WithTxn(txn5, ExecuteTxn(*bustub, _var, _txn,
                             "UPDATE maintable SET b = b + 1 WHERE a = 3"));
    WithTxn(txn5,
            QueryShowResult(
                *bustub, _var, _txn, "SELECT a, b FROM maintable ORDER BY a, b",
...
TEST(TxnAbortTest, AbortIndexTest) { // NOLINT
  fmt::println(stderr, "--- AbortIndexTest1: Simple Abort ---");
  {
    ...
    WithTxn(txn1, AbortTxn(*bustub, _var, _txn));
    TxnMgrDbg("after abort", bustub->txn_manager_.get(), table_info.get(),
              table_info->table_.get());
    auto txn2 = BeginTxn(*bustub, "txn2");
    WithTxn(
        txn2,
