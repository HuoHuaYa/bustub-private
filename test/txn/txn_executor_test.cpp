...
TEST(TxnExecutorTest, DISABLED_InsertTest) { // NOLINT
  ...
  TxnMgrDbg("after insertion", bustub->txn_manager_.get(), table_info.get(),
...
TEST(TxnExecutorTest, DISABLED_InsertCommitTest) { // NOLINT
  ...
  TxnMgrDbg("after commit txn1", bustub->txn_manager_.get(), table_info.get(),
            table_info->table_.get());
  ...
  TxnMgrDbg("after insert into txn3", bustub->txn_manager_.get(),
  ...
  TxnMgrDbg("after commit txn2", bustub->txn_manager_.get(), table_info.get(),
            table_info->table_.get());
...
TEST(TxnExecutorTest, DISABLED_InsertDeleteTest) { // NOLINT
  ...
  TxnMgrDbg("after 3 insert + 1 delete", bustub->txn_manager_.get(),
            table_info.get(), table_info->table_.get());
  fmt::println(stderr, "A: check scan txn1");
  ...
  WithTxn(txn1, CommitTxn(*bustub, _var, _txn));
  TxnMgrDbg("after commit", bustub->txn_manager_.get(), table_info.get(),
            table_info->table_.get());
  auto txn_ref = BeginTxn(*bustub, "txn_ref");
  auto txn2 = BeginTxn(*bustub, "txn2");
  fmt::println(stderr, "B: check scan txn2");
  ...
  TxnMgrDbg("after txn2 delete", bustub->txn_manager_.get(), table_info.get(),
            table_info->table_.get());
  ...
  TxnMgrDbg("after txn4 modification", bustub->txn_manager_.get(),
            table_info.get(), table_info->table_.get());
...
TEST(TxnExecutorTest, DISABLED_InsertDeleteConflictTest) { // NOLINT
  ...
  TxnMgrDbg("after txn3 tainted", bustub->txn_manager_.get(), table_info.get(),
            table_info->table_.get());
  auto txn4 = BeginTxn(*bustub, "txn4");
  ...
  WithTxn(txn6,
          ExecuteTxn(*bustub, _var, _txn, "DELETE FROM maintable WHERE a = 5"));
  TxnMgrDbg("after txn6 deletes", bustub->txn_manager_.get(), table_info.get(),
            table_info->table_.get());
  WithTxn(txn6, CommitTxn(*bustub, _var, _txn));
  TxnMgrDbg("after txn6 commits", bustub->txn_manager_.get(), table_info.get(),
            table_info->table_.get());
...
TEST(TxnExecutorTest, DISABLED_GenerateUndoLogTest) {
  {
    ...
    auto undo_log =
        GenerateNewUndoLog(schema.get(), &base_tuple, &target_tuple, 0, {});

    auto tuple =
        ReconstructTuple(schema.get(), target_tuple, {0, false}, {undo_log});
  ...
  }
  {
    ...
    auto base_tuple = Tuple{{Int(0), Double(0.0), Bool(true)}, schema.get()};
    auto undo_log =
        GenerateNewUndoLog(schema.get(), &base_tuple, nullptr, 0, {});

    auto tuple =
        ReconstructTuple(schema.get(), base_tuple, {0, true}, {undo_log});
    ASSERT_TRUE(tuple.has_value());
    ASSERT_TRUE(IsTupleContentEqual(*tuple, base_tuple));
  }
  {
    // This case is only relevant after task 4.2, where an insert may happen on
    // a tombstone. Before task 4.2, insert always generates a new tuple in
    // table heap without any undo logs.
    fmt::println(stderr, "--- GenerateUndoLogTest: Simple insert ---");
    auto schema = ParseCreateStatement("a integer,b double,c boolean");
    auto partial_schema = ParseCreateStatement("");
    auto target_tuple = Tuple{{Int(0), Double(1.0), Bool(false)}, schema.get()};
    auto undo_log =
        GenerateNewUndoLog(schema.get(), nullptr, &target_tuple, 0, {});
  ...
  }
  {
    ...
    auto partial_schema = ParseCreateStatement("b double,c boolean");
    auto base_tuple = Tuple{{Int(0), Double(0.0), Bool(true)}, schema.get()};
    auto intermediate_tuple =
        Tuple{{Int(0), Double(0.0), Bool(false)}, schema.get()};
    auto undo_log = GenerateNewUndoLog(schema.get(), &base_tuple,
                                       &intermediate_tuple, 0, {});
    auto target_tuple = Tuple{{Int(0), Double(1.0), Bool(false)}, schema.get()};
    auto updated_undo_log = GenerateUpdatedUndoLog(
        schema.get(), &intermediate_tuple, &target_tuple, undo_log);

    auto tuple = ReconstructTuple(schema.get(), target_tuple, {0, false},
                                  {updated_undo_log});
    ASSERT_TRUE(tuple.has_value());
    ASSERT_TRUE(IsTupleContentEqual(*tuple, base_tuple));
  }
  {
    fmt::println(stderr,
                 "--- GenerateUndoLogTest: Update then delete in a txn ---");
    auto schema = ParseCreateStatement("a integer,b double,c boolean");
    auto partial_schema = ParseCreateStatement("b double,c boolean");
    auto base_tuple = Tuple{{Int(0), Double(0.0), Bool(true)}, schema.get()};
    auto target_tuple = Tuple{{Int(0), Double(1.0), Bool(false)}, schema.get()};
    auto undo_log =
        GenerateNewUndoLog(schema.get(), &base_tuple, &target_tuple, 0, {});
    auto updated_undo_log =
        GenerateUpdatedUndoLog(schema.get(), &target_tuple, nullptr, undo_log);

    auto tuple = ReconstructTuple(schema.get(), target_tuple, {0, true},
                                  {updated_undo_log});
    ASSERT_TRUE(tuple.has_value());
    ASSERT_TRUE(IsTupleContentEqual(*tuple, base_tuple));
  }
  {
    ...
    // table heap without any undo logs.
    fmt::println(stderr,
                 "--- GenerateUndoLogTest: Insert then update in a txn ---");
    auto schema = ParseCreateStatement("a integer,b double,c boolean");
    auto partial_schema = ParseCreateStatement("");
    auto intermediate_tuple =
        Tuple{{Int(0), Double(0.0), Bool(false)}, schema.get()};
    auto undo_log =
        GenerateNewUndoLog(schema.get(), nullptr, &intermediate_tuple, 0, {});
    auto target_tuple = Tuple{{Int(0), Double(1.0), Bool(false)}, schema.get()};
    auto updated_undo_log = GenerateUpdatedUndoLog(
        schema.get(), &intermediate_tuple, &target_tuple, undo_log);
  ...
  }
  {
    ...
    // table heap without any undo logs.
    fmt::println(stderr,
                 "--- GenerateUndoLogTest: Insert then delete in a txn ---");
    auto schema = ParseCreateStatement("a integer,b double,c boolean");
    auto partial_schema = ParseCreateStatement("");
    auto intermediate_tuple =
        Tuple{{Int(0), Double(0.0), Bool(false)}, schema.get()};
    auto undo_log =
        GenerateNewUndoLog(schema.get(), nullptr, &intermediate_tuple, 0, {});
    auto updated_undo_log = GenerateUpdatedUndoLog(
        schema.get(), &intermediate_tuple, nullptr, undo_log);

    auto tuple = ReconstructTuple(schema.get(), intermediate_tuple, {0, true},
                                  {updated_undo_log});
    ASSERT_FALSE(tuple.has_value());
  }
  {
    ...
    // table heap without any undo logs.
    fmt::println(stderr,
                 "--- GenerateUndoLogTest: Delete then insert in a txn ---");
    auto schema = ParseCreateStatement("a integer,b double,c boolean");
    auto partial_schema = ParseCreateStatement("b double,c boolean");
    auto base_tuple = Tuple{{Int(0), Double(0.0), Bool(true)}, schema.get()};
    auto undo_log =
    ...
    auto target_tuple = Tuple{{Int(0), Double(1.0), Bool(false)}, schema.get()};
    auto updated_undo_log =
        GenerateUpdatedUndoLog(schema.get(), nullptr, &target_tuple, undo_log);
...
TEST(TxnExecutorTest, DISABLED_UpdateTest1) { // NOLINT
  ...
  TxnMgrDbg("after insert", bustub->txn_manager_.get(), table_info.get(),
            table_info->table_.get());
  ...
  TxnMgrDbg("after update", bustub->txn_manager_.get(), table_info.get(),
            table_info->table_.get());
  WithTxn(txn1,
...
TEST(TxnExecutorTest, DISABLED_UpdateTest2) { // NOLINT
  ...
  TxnMgrDbg("after insert and commit", bustub->txn_manager_.get(),
            table_info.get(), table_info->table_.get());
  auto txn1 = BeginTxn(*bustub, "txn1");
  auto txn_ref = BeginTxn(*bustub, "txn_ref");
...
TEST(TxnExecutorTest, DISABLED_UpdateConflict) { // NOLINT
  {
    ...
    WithTxn(txn0, CommitTxn(*bustub, _var, _txn));
    auto txn_ref = BeginTxn(*bustub, "txn_ref");
    TxnMgrDbg("after initialize", bustub->txn_manager_.get(), table_info.get(),
              table_info->table_.get());
    auto txn1 = BeginTxn(*bustub, "txn1");
    auto txn2 = BeginTxn(*bustub, "txn2");
    WithTxn(txn1, ExecuteTxn(*bustub, _var, _txn, "UPDATE table1 SET a = 1"));
    TxnMgrDbg("after 1st update", bustub->txn_manager_.get(), table_info.get(),
              table_info->table_.get());
    WithTxn(txn2,
            ExecuteTxnTainted(*bustub, _var, _txn, "UPDATE table1 SET b = 2"));
    TxnMgrDbg("after txn tainted", bustub->txn_manager_.get(), table_info.get(),
              table_info->table_.get());
    WithTxn(txn1, CommitTxn(*bustub, _var, _txn));
    TxnMgrDbg("after commit", bustub->txn_manager_.get(), table_info.get(),
              table_info->table_.get());
  ...
  }
  {
    ...
    TxnMgrDbg("after updates", bustub->txn_manager_.get(), table_info.get(),
              table_info->table_.get());
    WithTxn(txn3, ExecuteTxnTainted(*bustub, _var, _txn,
                                    "UPDATE table1 SET b = 2 WHERE a = 0"));
    TxnMgrDbg("after txn3 tainted", bustub->txn_manager_.get(),
              table_info.get(), table_info->table_.get());
    WithTxn(txn4, ExecuteTxnTainted(*bustub, _var, _txn,
                                    "UPDATE table1 SET b = 2 WHERE a = 1"));
    TxnMgrDbg("after txn4 tainted", bustub->txn_manager_.get(),
              table_info.get(), table_info->table_.get());
    WithTxn(txn2, CommitTxn(*bustub, _var, _txn));
    TxnMgrDbg("after commit", bustub->txn_manager_.get(), table_info.get(),
              table_info->table_.get());
...
TEST(TxnExecutorTest, DISABLED_GarbageCollectionWithTainted) { // NOLINT
  ...
  WithTxn(txn6, ExecuteTxnTainted(*bustub, _var, _txn,
                                  "DELETE FROM table1 WHERE a = 11"));
  TxnMgrDbg("after txn5 + txn6 tainted", bustub->txn_manager_.get(),
            table_info.get(), table_info->table_.get());
  GarbageCollection(*bustub);
  TxnMgrDbg("after garbage collection", bustub->txn_manager_.get(),
            table_info.get(), table_info->table_.get());
