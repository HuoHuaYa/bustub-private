  ...
  for (int n = 0; n < trials; n++) {
    ...
    WithTxn(query_txn,
            QueryShowResult(*bustub, _var, _txn, "SELECT * FROM maintable",
                            expected_rows));
    auto entry =
        TableHeapEntry(*bustub, bustub->catalog_->GetTable("maintable").get());
...
TEST(TxnIndexTest, DISABLED_IndexConcurrentUpdateTest) { // NOLINT
  ...
  for (int n = 0; n < trials; n++) {
    ...
    TableHeapEntryNoMoreThan(
        *bustub, bustub->catalog_->GetTable("maintable").get(), number_cnt);
