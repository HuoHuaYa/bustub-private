...
TEST(TxnScanTest, CollectUndoLogTest) { // NOLINT
  ...
  auto rid0 = *table_info->table_->InsertTuple(
      TupleMeta{txn0->GetCommitTs(), false},
  ...
  auto rid2 = *table_info->table_->InsertTuple(
      TupleMeta{txn2->GetCommitTs(), false},
  ...
  auto rid4 = *table_info->table_->InsertTuple(
      TupleMeta{txn3->GetCommitTs(), false},
      Tuple{{Int(4), Double(4.0), BoolNull()}, schema.get()});

  auto undo_link4 = txn3->AppendUndoLog(
      UndoLog{false,
              {true, true, false},
              Tuple{{Int(3), Double(3.0)}, modify_schema.get()},
              txn2->GetCommitTs(),
              {}});
  bustub->txn_manager_->UpdateUndoLink(rid4, undo_link4, nullptr);

  auto rid5 = *table_info->table_->InsertTuple(
      TupleMeta{txn3->GetCommitTs(), true},
      Tuple{{Int(2), Double(2.0), BoolNull()}, schema.get()});
  ...
  auto rid6 = *table_info->table_->InsertTuple(
      TupleMeta{txn3->GetCommitTs(), false},
      Tuple{{Int(4), Double(4.0), BoolNull()}, schema.get()});

  auto undo_link6 = txn1->AppendUndoLog(
      UndoLog{false,
              {true, true, true},
              Tuple{{Int(1), Double(1.0), BoolNull()}, schema.get()},
              txn0->GetCommitTs(),
              {}});
  auto undo_link7 = txn3->AppendUndoLog(UndoLog{true,
                                                {false, false, false},
                                                Tuple::Empty(),
                                                txn1->GetCommitTs(),
                                                undo_link6});
  ...
  auto rid7 = *table_info->table_->InsertTuple(
      TupleMeta{txn_to_inspect->GetTransactionTempTs(), false},
      Tuple{{Int(100), Double(100.0), BoolNull()}, schema.get()});
  auto rid8 = *table_info->table_->InsertTuple(
      TupleMeta{txn_to_inspect->GetTransactionTempTs(), false},
      Tuple{{Int(100), Double(100.0), BoolNull()}, schema.get()});
  ...
  auto rid9 = *table_info->table_->InsertTuple(
  ...
  auto undo_link9 = txn1->AppendUndoLog(
      UndoLog{false,
              ...
              txn0->GetCommitTs(),
              {}});
  auto undo_link10 = txn4->AppendUndoLog(
      UndoLog{false,
              {true, true, false},
  ...
  TxnMgrDbg("before verify scan", bustub->txn_manager_.get(), table_info.get(),
  ...
  auto undo_logs_0_for_txn_to_inspect =
      CollectUndoLogs(rid0, tuple_res_0.first, tuple_res_0.second,
                      bustub->txn_manager_->GetUndoLink(rid0), txn_to_inspect,
                      bustub->txn_manager_.get());
  auto undo_logs_1_for_txn_to_inspect =
      CollectUndoLogs(rid1, tuple_res_1.first, tuple_res_1.second,
                      bustub->txn_manager_->GetUndoLink(rid1), txn_to_inspect,
                      bustub->txn_manager_.get());
  auto undo_logs_2_for_txn_to_inspect =
      CollectUndoLogs(rid2, tuple_res_2.first, tuple_res_2.second,
                      bustub->txn_manager_->GetUndoLink(rid2), txn_to_inspect,
                      bustub->txn_manager_.get());
  auto undo_logs_3_for_txn_to_inspect =
      CollectUndoLogs(rid3, tuple_res_3.first, tuple_res_3.second,
                      bustub->txn_manager_->GetUndoLink(rid3), txn_to_inspect,
                      bustub->txn_manager_.get());
  auto undo_logs_4_for_txn_to_inspect =
      CollectUndoLogs(rid4, tuple_res_4.first, tuple_res_4.second,
                      bustub->txn_manager_->GetUndoLink(rid4), txn_to_inspect,
                      bustub->txn_manager_.get());
  auto undo_logs_5_for_txn_to_inspect =
      CollectUndoLogs(rid5, tuple_res_5.first, tuple_res_5.second,
                      bustub->txn_manager_->GetUndoLink(rid5), txn_to_inspect,
                      bustub->txn_manager_.get());
  auto undo_logs_6_for_txn_to_inspect =
      CollectUndoLogs(rid6, tuple_res_6.first, tuple_res_6.second,
                      bustub->txn_manager_->GetUndoLink(rid6), txn_to_inspect,
                      bustub->txn_manager_.get());
  auto undo_logs_7_for_txn_to_inspect =
      CollectUndoLogs(rid7, tuple_res_7.first, tuple_res_7.second,
                      bustub->txn_manager_->GetUndoLink(rid7), txn_to_inspect,
                      bustub->txn_manager_.get());
  auto undo_logs_8_for_txn_to_inspect =
      CollectUndoLogs(rid8, tuple_res_8.first, tuple_res_8.second,
                      bustub->txn_manager_->GetUndoLink(rid8), txn_to_inspect,
                      bustub->txn_manager_.get());
  auto undo_logs_9_for_txn_to_inspect =
      CollectUndoLogs(rid9, tuple_res_9.first, tuple_res_9.second,
                      bustub->txn_manager_->GetUndoLink(rid9), txn_to_inspect,
                      bustub->txn_manager_.get());
...

  {
    ...
    auto tuple =
        ReconstructTuple(schema.get(), tuple_res_0.second, tuple_res_0.first,
                         *undo_logs_0_for_txn_to_inspect);
    ASSERT_TRUE(tuple.has_value());
...

  {
    ...
    auto tuple =
        ReconstructTuple(schema.get(), tuple_res_1.second, tuple_res_1.first,
                         *undo_logs_1_for_txn_to_inspect);
    ASSERT_TRUE(tuple.has_value());
  ...
  }
  {
    ...
    auto tuple =
        ReconstructTuple(schema.get(), tuple_res_2.second, tuple_res_2.first,
                         *undo_logs_2_for_txn_to_inspect);
    ASSERT_TRUE(tuple.has_value());
    VerifyTuple(schema.get(), *tuple, {Int(2), Double(2.0), BoolNull()});
  }
  ...
  { ASSERT_FALSE(undo_logs_4_for_txn_to_inspect.has_value()); }
  {
    ...
    auto tuple =
        ReconstructTuple(schema.get(), tuple_res_5.second, tuple_res_5.first,
                         *undo_logs_5_for_txn_to_inspect);
    ASSERT_TRUE(tuple.has_value());
    VerifyTuple(schema.get(), *tuple, {Int(2), Double(2.0), BoolNull()});
  }
  {
    ...
    auto tuple =
        ReconstructTuple(schema.get(), tuple_res_6.second, tuple_res_6.first,
                         *undo_logs_6_for_txn_to_inspect);
    ASSERT_FALSE(tuple.has_value());
  }
  {
    ASSERT_TRUE(undo_logs_7_for_txn_to_inspect.has_value());
    auto tuple =
        ReconstructTuple(schema.get(), tuple_res_7.second, tuple_res_7.first,
                         *undo_logs_7_for_txn_to_inspect);
    ASSERT_TRUE(tuple.has_value());
  ...
  }
  {
    ...
    auto tuple =
        ReconstructTuple(schema.get(), tuple_res_8.second, tuple_res_8.first,
                         *undo_logs_8_for_txn_to_inspect);
    ASSERT_TRUE(tuple.has_value());
    VerifyTuple(schema.get(), *tuple, {Int(100), Double(100.0), BoolNull()});
  }
  {
    ...
    auto tuple =
        ReconstructTuple(schema.get(), tuple_res_9.second, tuple_res_9.first,
                         *undo_logs_9_for_txn_to_inspect);
    ASSERT_TRUE(tuple.has_value());
    VerifyTuple(schema.get(), *tuple, {Int(1), Double(1.0), BoolNull()});
  }
}
