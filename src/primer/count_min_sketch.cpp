...
CountMinSketch<KeyType>::CountMinSketch(uint32_t width, uint32_t depth)
  ...
  // counts_.resize(depth, std::vector<std::atomic<size_t>>(width,
  // 0));这个写法是拷贝一个vector<atomic...> 但是atomic不支持拷贝所以不行吧

  counts_.reserve(
      depth); // 只能是reserve，因为resize是直接赋予，而reverse是先预定，后面等待手动赋予
...
void CountMinSketch<KeyType>::Merge(const CountMinSketch<KeyType> &other) {
  ...
  for (size_t i = 0; i < depth_; i++) {
    for (size_t j = 0; j < width_; j++) {
      counts_[i][j].fetch_add(other.counts_[i][j].load(),
...
auto CountMinSketch<KeyType>::Count(const KeyType &item) const -> uint32_t {
  ...
  for (size_t i = 0; i < depth_; i++) {
    min_val = std::min(
        min_val,
        static_cast<uint32_t>(counts_[i][hash_functions_[i](item)].load()));
  }
  return min_val;
}
...
auto CountMinSketch<KeyType>::TopK(uint16_t k,
  ...
  std::sort(can_val.begin(), can_val.end(),
  ...
  return std::vector<std::pair<KeyType, uint32_t>>(can_val.begin(),
                                                   can_val.begin() + numbers);
}
