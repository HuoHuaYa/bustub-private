...
IndexStatement::IndexStatement(
    ...
    : BoundStatement(StatementType::INDEX_STATEMENT),
      ...
      cols_(std::move(cols)), index_type_(std::move(index_type)),
      col_options_(std::move(col_options)), options_(std::move(options)) {}

auto IndexStatement::ToString() const -> std::string {
  return fmt::format("BoundIndex {{ index_name={}, table={}, cols={}, "
                     "using={}, col_options=[{}], options=[{}] }}",
                     index_name_, *table_, cols_, index_type_,
                     fmt::join(col_options_, ","), fmt::join(options_, ","));
}
