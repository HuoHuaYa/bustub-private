...
inline auto MockRandomValuesEnabled() -> bool {
  if (const char *v = std::getenv("BUSTUB_ENABLE_RANDOM");
      (v != nullptr) && (*v != 0)) {
...
inline auto MockSeed() -> uint64_t {
  if (const char *s = std::getenv("BUSTUB_MOCKSCAN_SEED");
      (s != nullptr) && (*s != 0)) {
    try {
      return std::stoull(s);
    } catch (...) {
  ...
  if (const char *s = std::getenv("BUSTUB_TABLEGEN_SEED");
      (s != nullptr) && (*s != 0)) {
    try {
      return std::stoull(s);
    } catch (...) {
...
inline auto MakeTableRng(const std::string &table) -> std::mt19937_64 {
  ...
  std::seed_seq seq{static_cast<uint32_t>(seed),
                    static_cast<uint32_t>(seed >> 32), static_cast<uint32_t>(h),
                    static_cast<uint32_t>(h >> 32)};
  return std::mt19937_64(seq);
}

// Generic random value for a column (keeps types/sensible ranges)
inline auto RandomValueForColumn(const Column &col, std::mt19937_64 &rng)
    -> Value {
  switch (col.GetType()) {
  case TypeId::INTEGER: {
...
static const char *ta_oh_2024[] = {
    "Friday",    "Thursday", "Friday",  "Wednesday", "Thursday",
    "Yesterday", "Monday",   "Tuesday", "Tuesday",   "Monday"};

static const char *ta_oh_2024_fall[] = {"Wednesday", "Thursday", "Tuesday",
                                        "Monday",    "Friday",   "Thursday",
                                        "Tuesday",   "Friday"};

static const char *ta_oh_2025_spring[] = {"Friday",  "Monday", "Wednesday",
                                          "Tuesday", "Friday", "Thursday",
                                          "Monday",  "Tuesday"};

static const char *ta_oh_2025_fall[] = {"Tuesday", "Monday",   "Thursday",
...
const char *mock_table_list[] = {
    ...
    "__mock_table_tas_2024_fall", "__mock_table_tas_2025_spring",
    "__mock_table_tas_2025_fall", "__mock_agg_input_small",
    "__mock_agg_input_big", "__mock_external_merge_sort_input",
...
auto GetMockTableSchemaOf(const std::string &table) -> Schema {
  ...
  if (table == "__mock_table_4") {
    return Schema{std::vector{Column{"colG", TypeId::INTEGER},
                              {Column{"colH", TypeId::VARCHAR, 128}}}};
  }
  ...
  if (table == "__mock_graph") {
    // Note(f24): Make `src_label` and `dst_label` INTEGER because the current
    // external merge sort implementation only supports sorting on fixed-length
    // data. Should be VARCHAR(8) if var-length data sorting is supported in the
    // future.
    return Schema{std::vector{Column{"src", TypeId::INTEGER},
                              Column{"dst", TypeId::INTEGER},
                              Column{"src_label", TypeId::INTEGER},
                              Column{"dst_label", TypeId::INTEGER},
...
auto GetFunctionOf(const MockScanPlanNode *plan)
  ...
  if (table == "__mock_table_tas_2024_fall") {
    return [plan](size_t cursor) {
      std::vector<Value> values{};
      values.push_back(
          ValueFactory::GetVarcharValue(ta_list_2024_fall[cursor]));
  ...
  if (table == "__mock_table_tas_2025_spring") {
    return [plan](size_t cursor) {
      std::vector<Value> values{};
      values.push_back(
          ValueFactory::GetVarcharValue(ta_list_2025_spring[cursor]));
      values.push_back(
          ValueFactory::GetVarcharValue(ta_oh_2025_spring[cursor]));
      return Tuple{values, &plan->OutputSchema()};
    };
  }
  ...
  if (table == "__mock_table_tas_2025_fall") {
    return [plan](size_t cursor) {
      std::vector<Value> values{};
      values.push_back(
          ValueFactory::GetVarcharValue(ta_list_2025_fall[cursor]));
  ...
  if (table == "__mock_graph") {
    return [plan](size_t cursor) {
      ...
      // Note(f24): Use INTEGER for `src_label` and `dst_label` based on current
      // external merge sort.
 ...
 */
auto MockScanExecutor::Next(std::vector<bustub::Tuple> *tuple_batch,
