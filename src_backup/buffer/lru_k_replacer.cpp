 ...
 * k-distance. If multiple frames have inf backward k-distance, then evict frame
 * whose oldest timestamp is furthest in the past.
 *
 * Successful eviction of a frame should decrement the size of replacer and
 * remove the frame's access history.
 *
 * @return the frame ID if a frame is successfully evicted, or `std::nullopt` if
 ...
 * frame with largest backward k-distance. This function removes specified frame
 * id, no matter what its backward k-distance is.
 *
