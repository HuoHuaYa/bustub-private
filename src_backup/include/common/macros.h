...
// An internal stream, used to take C++ stream-style parameters for display, and
// exit the program with `std::abort`.
class LogFatalStream {
public:
  LogFatalStream(const char *file, int line) : file_(file), line_(line) {}

  ~LogFatalStream() {
    std::cerr << file_ << ":" << line_ << ": " << log_stream_.str()
              << std::endl;
    std::abort();
  }

  template <typename T> auto operator<<(const T &val) -> LogFatalStream & {
    log_stream_ << val;
    return *this;
  }
...
#define BUSTUB_ASSERT_AND_LOG(expr) /*NOLINT*/                                 \
  if (bool val = (expr); !val)                                                 \
    internal::LogFatalStream { /*NOLINT*/                                      \
