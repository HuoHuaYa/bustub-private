...
/**
 * @brief A probabilistic data structure that implements the ordered set
 * abstract data type.
 *
 * The skip list is implemented as a linked list of nodes. Each node has a list
 * of forward links. The number of forward links is determined by a geometric
 * distribution. The skip list maintains a header node that is always at the
 * maximum height of the skip list. The skip list is always sorted by the key
...
public:
  /**  @brief Constructs an empty skip list with an optional custom comparison
   * function. */
  ...
  /**
   * We disable both copy & move operators/constructors as the skip list
   * contains a mutex that cannot be safely moved/copied while other threads may
   * be using it.
   */
...
private:
  ...
  //
  // To give you an idea, the following are some common helper functions that
  ...
  /** @brief Comparison function used to check key orders and keep the skip list
   * sorted. */
  Compare compare_;

  /**
   * @brief Header consists of a list of forward links for level 0 to `MaxHeight
   * - 1`. The forward links at level higher than current `height_` are
   * `nullptr`.
   */
 ...
 */
SKIPLIST_TEMPLATE_ARGUMENTS struct SkipList<K, Compare, MaxHeight,
                                            Seed>::SkipNode {
  /**
   * @brief Constructs a skip list node with height number of links and given
   * key.
   *
   * This constructor is used both for creating regular nodes (with key) and the
   * header node (without key).
   *
   ...
   *
   * Note: `links_[0]` is the lowest level link, and `links_[links_.size()-1]`
   * is the highest level link.
   *
   * We use `std::shared_ptr` to manage the memory of the next node. This is
   * because the next node can be shared by multiple links. We also use
   * `std::vector` instead of a flexible array member for simplicity instead of
   * performance.
   */
