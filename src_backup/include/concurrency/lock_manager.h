...
public:
  ...
  public:
    ...
    LockMode lock_mode_;
    /** Oid of the table for a table lock; oid of the table to which the row
     * belongs for a row lock */
    table_oid_t oid_;
   ...
   *
   *    Similarly, X/IX locks on rows are not allowed if the Transaction State
   * is SHRINKING, and any such attempt should set the TransactionState as
