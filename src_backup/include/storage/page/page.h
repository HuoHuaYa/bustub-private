...
public:
  ...
  Page()
      : data_(static_cast<size_t>(BUSTUB_PAGE_SIZE), 0),
...
private:
   ...
   *
   * Zeroes out the data that is held within the frame and sets all fields to
   * default values.
   */
   ...
   * However, in order to allow address sanitizer to detect buffer overflow, we
   * store it as a vector.
   *
   * Note that friend classes should make sure not increase the size of this
   * data field.
   */
  ...
  /** @brief True if the page is dirty, i.e. it is different from its
   * corresponding page on disk. */
